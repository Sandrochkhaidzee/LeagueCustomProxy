...
- **Client tests** live under `tests/` (separate root from `src/`). Run with `npm test`. The existing 42 tests cover core logic, tracking state machine, audio EMA math, device list filtering, and the session-flow integration.
- **Server tests** live under `server/tests/`. Run with `cd server && npm test`. 33 tests cover room management, TURN credential generation (both coturn-HMAC and Cloudflare paths), and volume math.
